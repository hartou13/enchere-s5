...
select v_enchere_status.*, mise.utilisateurid, nomlot from (select utilisateurid, enchereid from mise group by utilisateurid, enchereid) as mise join v_enchere_status on enchereid=v_enchere_status.id join lot on lotid=lot.id;

create or replace view v_enchere_recherche as 
select v_enchere_status.*, lot.nomlot from v_enchere_status join lot on lotid=lot.id ;
